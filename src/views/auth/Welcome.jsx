  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#070d18] text-slate-100 selection:bg-cyan-300 selection:text-slate-950">
      ...
      <main className="relative z-10">
        ...
        <section id="planes" className="border-y border-white/[0.07] bg-[#0a1221] py-20 sm:py-24"><div className="mx-auto max-w-7xl px-5 sm:px-8"><div className="grid items-center gap-12 lg:grid-cols-[0.9fr_1.1fr]"><div><p className="mb-3 text-xs font-black uppercase tracking-[0.2em] text-cyan-300">Crece a tu ritmo</p><h2 className="text-3xl font-black tracking-tight text-white sm:text-4xl">Empieza con lo esencial. Amplía cuando tu despacho lo necesite.</h2><p className="mt-5 leading-7 text-slate-400">Estamos preparando planes adaptados al tamaño de cada cartera. En la demo definimos el nivel adecuado sin pagar por módulos que todavía no utilizas.</p><button type="button" onClick={() => goTo('/demo')} className="mt-8 inline-flex items-center gap-2 rounded-xl border border-cyan-300/30 px-5 py-3 text-sm font-bold text-cyan-200 transition hover:bg-cyan-300/10">Solicitar orientación <ArrowRight size={15} /></button></div><div className="grid gap-4 sm:grid-cols-3"><div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5"><p className="text-xs font-bold text-cyan-300">Starter</p><p className="mt-3 text-2xl font-black text-white">Para comenzar</p><p className="mt-3 text-sm leading-6 text-slate-500">Gestión de fincas, propietarios, documentos e incidencias.</p></div><div className="rounded-2xl border border-cyan-300/30 bg-cyan-300/[0.08] p-5 shadow-lg shadow-cyan-950/30"><p className="text-xs font-bold text-cyan-200">Profesional</p><p className="mt-3 text-2xl font-black text-white">Para crecer</p><p className="mt-3 text-sm leading-6 text-slate-400">Más capacidad, cuotas, agenda, CRM y asistencia de IA.</p></div><div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5"><p className="text-xs font-bold text-cyan-300">Enterprise</p><p className="mt-3 text-2xl font-black text-white">A medida</p><p className="mt-3 text-sm leading-6 text-slate-500">Migración, soporte e integraciones para grandes despachos.</p></div></div></div></div></section>

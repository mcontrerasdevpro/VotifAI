...
import { Shield, Landmark, MapPin, FileUp, CheckCircle, ArrowRight, Loader2 } from 'lucide-react';
...
export default function AltaEmpresa() {
  ...
  const [ubicacion, setUbicacion] = useState(''); // Domicilio Social
  const [archivoPDF, setArchivoPDF] = useState(null); // Escrituras / Estatutos
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (respuesta.ok) {
        ...
        // Despachamos la entidad al estado global con tipo fijo 'empresa'
        dispatch({ 
          type: 'AÑADIR_ENTIDAD', 
          payload: {
            id: resultadoBack.id, 
            nombre: resultadoBack.nombre,
            tipo: 'empresa', // Forzado a empresa para segmentar el panel correcto después
            cif: resultadoBack.cif,
            direccion: resultadoBack.direccion,
            estado: 'Asamblea Programada — HOY 18:00', // Vocabulario corporativo (Asamblea en vez de Junta)
            socios: [] // Cambiado de propietarios a socios
          }
        });
        ...
        setProcesando(false);
        alert("❌ Error: Este CIF empresarial ya está registrado.");
      }
    } catch (error) {
      console.error("Fallo de inserción relacional corporativa:", error);
      setProcesando(false);
    ...
    <div className="h-screen w-screen bg-slate-950 text-slate-100 flex overflow-hidden font-sans antialiased">
      ...
      <div className="w-full lg:w-5/12 h-full bg-[#0d1117] border-r border-slate-900 p-8 flex flex-col justify-between overflow-y-auto custom-scrollbar">
        ...
        <div className="space-y-2 shrink-0">
          ...
          <h1 className="text-xl font-black tracking-tight text-white">Desplegar Nueva Sociedad</h1>
        </div>

        <form onSubmit={handleSubmit} className="flex-grow my-6 space-y-4 flex flex-col justify-center max-w-md w-full mx-auto">
          
          {/* Indicador estático de Marco Regulatorio Empresarial */}
          <div className="space-y-1.5">
            <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400">Marco Regulatorio</label>
            <div className="bg-slate-950 p-1 rounded-xl border border-slate-900">
              <div className="flex items-center justify-center gap-2 py-2 rounded-lg text-3xs font-black uppercase tracking-wider border bg-indigo-600/10 border-indigo-500 text-indigo-400">
                <Landmark size={14} /> Ley de Sociedades de Capital (LSC)
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              type="text" required placeholder="Ej: Inversiones Inmobiliarias S.L. o Tecnología S.A."
         ...
         {/* CIF y Ubicación en Grid */}
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-1 space-y-1.5">
              ...
              <input
                type="text" required placeholder="A1234567B" maxLength={9}
            ...
            <div className="col-span-2 space-y-1.5">
              <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400">Domicilio Social</label>
          ...
          {/* Capturador de PDF */}
          <div className="space-y-1.5">
            <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400">Estatutos Sociales / Acta (Opcional)</label>
            <label className="w-full border border-dashed border-slate-800 hover:border-slate-700 bg-slate-950 rounded-xl p-4 flex flex-col items-center justify-center gap-1.5 cursor-pointer transition-colors shadow-inner">
              ...
              <span className="text-4xs font-bold text-slate-400 uppercase tracking-wider truncate max-w-full px-2">
                {archivoPDF ? archivoPDF.name : 'Adjuntar Escritura o Estatutos de la Sociedad'}
              </span>
            </label>
          </div>
          ...
          >
            {procesando ? (
              ...
              <> <CheckCircle size={14} className="animate-bounce" /> Entorno Corporativo Activo </>
            ) : (
              <> Inicializar Sociedad <ArrowRight size={14} /> </>
            )}
          </button>
        </form>

        <div className="text-[9px] text-slate-600 font-medium border-t border-slate-900/60 pt-4 shrink-0">
          * Alta con cifrado HASH SHA-256 para prevenir la alteración posterior del libro de actas mercantil.
        </div>
      </div>
              ...
              {procesando ? (
                ...
                >
                  <p className="text-3xs font-mono text-indigo-400 font-bold uppercase tracking-widest animate-pulse">
                    Mapeando Estructura de Capital...
                  </p>
                  ...
                  <p className="text-[9px] text-slate-500 font-medium">
                    Generando registro mercantil inicial y libro de socios de la sociedad...
                  </p>
                </motion.div>
              ) : completado ? (
                <motion.div
                ...
                >
                  ...
                  <p className="text-3xs font-black uppercase text-white tracking-wider">Sociedad Vinculada Correctamente</p>
                  <p className="text-[10px] text-emerald-400/80 font-mono">UUID-SECURE EMPRESARIAL GENERADO</p>
                </motion.div>
              ) : (
                <motion.div
                ...
                >
                  ...
                  <div className="flex justify-between items-center text-4xs">
                    <span className="text-slate-400 font-medium">Identificador Fiscal (CIF):</span>
                    <span className="font-mono font-black text-indigo-400 uppercase">{cif || '—'}</span>
                  </div>
                  <div className="flex justify-between items-center text-4xs">
                    <span className="text-slate-400 font-medium">Domicilio Registrado:</span>
                    <span className="font-medium text-slate-300 truncate max-w-[200px]">{ubicacion || '—'}</span>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
